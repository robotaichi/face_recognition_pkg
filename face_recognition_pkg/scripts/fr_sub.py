...
file_output_path = '/home/limlab/catkin_ws/src/face_recognition_pkg/output/output_test.mp4'  #動画の書き込みパス
...
class openpose():
    ...
    def set_key_and_value(self): #キーリストと値リストの取得
    ...
    def set_params(self): #OpenPoseのパラメータ設定
        key_list, value_list = self.set_key_and_value() #キーリストと値リストの取得
...
class Subscribers(): #サブスクライバーのクラス
    def __init__(self): #コンストラクタと呼ばれる初期化のための関数（メソッド）
        ...
        self.subscriber = rospy.Subscriber('openpose_topic', face_recognition_message, self.callback)
        self.rate = rospy.Rate(1) #1秒間に2回データを受信する
    ...
    def callback(self, message): #サブスクライバーがメッセージを受信した際に実行されるcallback関数。messageにはパブリッシャーによって配信されたメッセージ（データ）が入る
        print("#########################################################################################################################################################################################################################################################################")
        # 受信したデータを出力する
        rospy.loginfo("受信：message = {}\n".format(message.openpose_message))



def main(): #メイン関数
    #初期化し、ノードの名前を設定
    rospy.init_node('fr_sub', anonymous=True)
    #クラスのインスタンス作成（クラス内の関数や変数を使えるようにする）
    sub = Subscribers()
    #sub.count += 1
    op_instance = openpose() #OpenPoseのインスタンス作成
    op_instance.op_start()
