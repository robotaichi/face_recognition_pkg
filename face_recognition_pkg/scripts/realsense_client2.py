...
class face_recognition():
    ...
    def face_detection_setting(self):  # 顔認識の設定
        ...
        message1, frames_count, diff = self.call_calculate_service(calculate_message)
        return frame_openpose, gray_image, message1, frames_count, diff
    ...
    def finish(self, frames_count):  # 終了処理
        print("\n終了")
        front_face_percentage = self.calculate_front_face_percentage(frames_count) #正面を向いた割合を計算
    ...
    def call_calculate_service(self, calculate_message):
        message1, frames_count, diff = self.cli.calculate_service_request(calculate_message)
        return message1, frames_count, diff



    def main_loop(self):  # メイン処理のループ
        while not rospy.is_shutdown():
            frame_openpose, gray_image, message1, frames_count, diff = self.face_detection_setting()  # 顔認識の設定
...
class cal_Client():  # クライアントのクラス
    ...
    def calculate_service_request(self, calculate_message):  # サービスのリクエスト
        ...
        try:
            ...
            return response.message1, response.frames_count, response.diff
